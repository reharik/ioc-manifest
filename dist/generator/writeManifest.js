...
const plansToLeanContainerContracts = (plans) => {
    const sortedPlans = [...plans].sort((a, b) => a.contractName.localeCompare(b.contractName));
    const out = {};
    for (const plan of sortedPlans) {
        const inner = {};
        const implNames = [...plan.implementations].sort((a, b) => a.implementationName.localeCompare(b.implementationName));
...
const serializeLeanImplementationBlock = (row) => {
    const lines = [
    ...
    if (row.configOverridesApplied !== undefined) {
        lines.push(`        configOverridesApplied: ${JSON.stringify(row.configOverridesApplied)},`);
    }
    if (row.dependencyContractNames !== undefined) {
        lines.push(`        dependencyContractNames: ${JSON.stringify(row.dependencyContractNames)},`);
    }
    return lines.join("\n");
};
const serializeLeanContractsObject = (lean) => {
...
const serializeMetadataBlock = (meta) => {
    ...
    if (meta.configOverridesApplied !== undefined) {
        lines.push(`      configOverridesApplied: ${JSON.stringify(meta.configOverridesApplied)},`);
    }
    if (meta.dependencyContractNames !== undefined) {
        lines.push(`      dependencyContractNames: ${JSON.stringify(meta.dependencyContractNames)},`);
    }
    return lines.join("\n");
};
const serializeRegistrationManifestValue = (manifest) => {
...
const serializeMainIocManifestSource = (lean, bundlesManifest, manifestImportFromPackage, importLines, moduleArrayLines) => {
...
`;
    ...
    const closing = bundlesManifest === undefined
        ? "}"
...
const serializeSupportManifestSource = (registrationManifest, bundleArraysInsight, manifestImportFromPackage) => {
    const header = `/* AUTO-GENERATED. DO NOT EDIT.
...
`;
    ...
    const insightLine = bundleArraysInsight === undefined
        ? "export const iocBundleArraysInsight: IocBundleArraysInsightManifest | undefined = undefined;"
        : `export const iocBundleArraysInsight: IocBundleArraysInsightManifest = ${JSON.stringify(bundleArraysInsight, null, 2)};`;
    return `${header}import type {
...
export const writeManifest = async (acceptedFactories, plans, bundlesPlan, bundleArraysInsight, manifestOutPath, manifestImportFromPackage) => {
    ...
    const iocRegistrationManifest = plansToIocContractManifest(plans, moduleIndexByPath);
    ...
    const mainSource = serializeMainIocManifestSource(leanContracts, iocBundlesManifest, manifestImportFromPackage, importLines, moduleArrayLines);
    const supportPath = path.join(path.dirname(manifestOutPath), "ioc-manifest.support.ts");
    const supportSource = serializeSupportManifestSource(iocRegistrationManifest, bundleArraysInsight, manifestImportFromPackage);
