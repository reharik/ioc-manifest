...
import { buildBundleReport, buildDiscoveryReport, buildInspectionReport, formatBundleReport, formatDiscoveryReport, formatInspectionReport, } from "../inspection/index.js";
import { runDiscoveryAnalysis } from "../inspection/runDiscoveryAnalysis.js";
const parseArgs = (argv) => {
    const args = argv.slice(2);
    if (args.length === 0) {
        throw new Error("Usage: ioc <inspect|bundles> [--discovery] [--config <path>]");
    }
    const command = args[0];
    if (command !== "inspect" && command !== "bundles") {
        throw new Error(`Unknown command ${JSON.stringify(command)}. Use inspect or bundles.`);
    }
    let iocConfigPath;
    let discovery = false;
...
const loadGeneratedManifestModules = async (iocConfigPath) => {
    ...
    const supportPath = path.join(path.dirname(manifestPath), "ioc-manifest.support.ts");
    const [main, support] = await Promise.all([
        import(pathToFileURL(manifestPath).href),
        import(pathToFileURL(supportPath).href),
    ]);
    return { main, support };
};
const main = async () => {
    const cli = parseArgs(process.argv);
    const { main: mainMod, support } = await loadGeneratedManifestModules(cli.iocConfigPath);
