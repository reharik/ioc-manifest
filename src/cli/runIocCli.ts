 ...
 *
 * Everything the CLI does lives here as an ordinary async function over an argv array; `ioc.ts` is
 * the executable shim that hands it `process.argv` and turns its return value into an exit code.
 * The split exists so a test can ask what a command PRINTS without paying for a process to print
 * it — see `test-support/runIocCliInProcess.ts` — while the process contract itself (argv from a
 * real command line, exit codes reaching a shell, stdout and stderr as separate descriptors) stays
 * covered by tests that spawn `ioc.ts` for real.
 *
...
/**
 * Runs one command and returns the exit code it earned.
 *
 * Returned rather than assigned to `process.exitCode`, so a caller that is not a process — a test,
 * or a future embedder — gets the same answer the shell would without the run leaving a mark on
 * the process it happened to run in.
 */
const dispatch = async (argv: readonly string[]): Promise<number> => {
  const parsed = parseIocCliArgv(argv);
  if (parsed.kind === "help") {
  ...
  if (parsed.kind === "validate") {
    ...
    return printValidateResult(result, cli.json);
  }
  ...
  if (parsed.kind === "explain") {
    ...
    return report.resolution.kind === "unknown" ? 1 : 0;
  }
 ...
 */
export const runIocCli = async (argv: readonly string[]): Promise<number> => {
  try {
