...
**Generation fails on 4.1 where it passed on 4.0.2** — 4.1 ranks dependency edges that cross into a composed package; before it, every key a composed manifest supplied was classified as an external and skipped, so those edges were ranked by nobody. The findings are real captive dependencies that have always been there and were never reported, not new breakage. Fix them rather than suppressing them: move the consumer behind a [scope opener](/concepts/scope-roots) so it is built per request, or give it a dependency that does not need request scope. `allowLifetimeInversion` is for inversions that are genuinely intended. See [Upgrading to 4.1](/concepts/lifetimes#lifetime-inversion-checks).

**A composed package's group members are reported as `UNRANKED, not cleared`** — the group hop was reached but the member's lifetime could not be read from anything this run loaded. It is disclosure, not a verdict: the edge was not checked. The usual cause is a stale composed manifest — regenerate the library, then the app. See [Across a composed boundary](/concepts/lifetimes#across-a-composed-boundary).

**Generation warns that a package "cannot claim `dependencyKeysComplete`"** — one or more accepted factories take their dependencies in a shape the keys cannot be read from, most often `(deps: Deps)` instead of `({ a, b }: Deps)`. They run fine; what they lack is a record of what they demand, which the lifetime check, the scope-root walk, and every composing app all read. The report names each one with the fix for its shape. See [dependency-key coverage](/reference/cli#ioc-generate-dependency-key-coverage) and [`dependencyKeyCoverage`](/config/reference#dependencykeycoverage).

**A composed blind-spot advisory persists after regenerating** — regenerating only helps when the package predated `dependencyKeys` entirely. If it emits keys but withholds `dependencyKeysComplete`, some factory in it cannot have its demands read, and that factory has to be fixed in *its* package. See [Composed blind spots](/concepts/scope-roots#composed-blind-spots).
